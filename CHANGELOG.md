...
- **New package icon.** The previous mark placed Unity's logo at the centre of
  the atom, which the Asset Store does not permit outside packages published by
  Unity themselves. It is now a game controller. Replaced everywhere it appears:
  the `.uitkx` file icon that ships in the package and is drawn in the builder's
  folder pane, the VS Code Marketplace icon and file-type icon, and the
  documentation site's logo and favicon.

- **Three game samples no longer ship in the Asset Store package.**
  `MarioGame`, `GalagaGame` and `DoomGame`, along with their three editor demo
  windows, are excluded from the store build. Their names and sprite art
  reference other companies' properties, which the Asset Store's content policy
  does not allow. `SnakeGame` and every other sample are unaffected, and the
  three remain in the repository for development.

### Added

- **`Third-Party Notices.txt`** at the package root, with the full licence text
  of the one third-party binary the package redistributes
  (`System.Collections.Immutable`, MIT). It also records what is NOT
  redistributed — the Newtonsoft dependency Unity resolves, and the Roslyn
  assemblies the generator is compiled against but which the editor supplies —
  and states that the package contains no third-party fonts, audio or imagery.
  `THIRDPARTY.md` remains the full index of every dependency used anywhere in
  the project, including development tooling.
...
- **An export named with a C# reserved keyword is now an error (`UITKX2114`).**
  ...
  `export Style default = …` produced uncompilable generated code and a storm of
  `CS` errors pointing at a file the author never wrote. Components were already
  shielded by the PascalCase rule &mdash; every C# keyword is lowercase &mdash;
  so the reachable hole was the camelCase kinds: style, utils and value exports.
  Now one precise error names the culprit.

  The comparison is ORDINAL, which is the whole point: `new` is reserved, `New`
  is a perfectly legal component name and stays one. Contextual keywords
  (`value`, `var`, `record`) are legal identifiers and are deliberately absent
  from the set. No valid file changes behaviour: code this rejects did not
  compile before either.
