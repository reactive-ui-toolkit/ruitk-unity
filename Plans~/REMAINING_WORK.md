...
| IL2CPP-BIND | `PropsHelper.Bind<TProps, TSource>` constrains `TSource` to `INotifyPropertyChanged` with no `class`, then does `source.PropertyChanged += Handler` - an event subscription through a constrained type parameter, the same shape that made IL2CPP refuse `MultiColumnLayoutTracker.Attach` (#253). Unproven: the reported failure was the tracker, and unlike the trackers this is PUBLIC API, so adding `class` narrows a consumer-visible constraint and needs the deprecate-for-one-minor treatment rather than a drive-by. Settle it empirically, not by reading. Trigger to revisit: the WebGL/IL2CPP CI job (`Plans~/GITHUB_ISSUES_252_259_PLAN.md` 10, item 4) - build a scene that calls `Bind` and see whether IL2CPP takes it. | `Shared/Props/PropsHelper.cs:138`, `:171` | #253 audit, 2026-09-20 |
