...
| SAMPLES-PLAYER | **A player build of ANY consumer project fails to compile.** `Samples/Components/EditorControlsDemoFunc/EditorControlsDemoFunc.uitkx` uses `<TwoPaneSplitView>`, whose `V.TwoPaneSplitView` factory is `#if UNITY_EDITOR` (`Shared/Core/V.cs:90`) because the control is `UnityEditor.UIElements.TwoPaneSplitView`. The file sits in `Ruitk.Samples`, whose asmdef is all-platforms, and it is NOT under a folder named `Editor`, so Unity compiles it into the player and the source generator - which cannot see the editor-only factory - reports `UITKX0109` plus `CS0103: The name 'TwoPaneSplitView' does not exist in the current context`. `Samples/` is a normal package folder (only `Samples~/` is hidden) and neither omit list excludes it, so it ships on BOTH the UPM and Asset Store channels. Its only consumer, `Samples/Showcase/Editor/EditorUitkxEditorControlsDemoWindow.cs`, IS under an `Editor` folder and is already editor-only - so the sample is the odd one out, not the window. Fix: give the sample an editor-only home (its own asmdef with `includePlatforms: ["Editor"]`, or a folder named `Editor`); either changes the FILE-keyed namespace, which is contained because that one window is the only importer. Found 2026-09-21 by the first IL2CPP build; nothing else in this repo has ever built a player, which is exactly why it survived. | `Samples/Components/EditorControlsDemoFunc/EditorControlsDemoFunc.uitkx`; `Shared/Core/V.cs:90`; `Samples/Ruitk.Samples.asmdef` `includePlatforms: []` | #253 verification, 2026-09-21 |
