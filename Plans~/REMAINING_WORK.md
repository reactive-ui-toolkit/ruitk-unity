...
| IL2CPP-BIND | **RESOLVED 2026-09-21 - not a defect.** `PropsHelper.Bind<TProps, TSource>` constrains `TSource` to `INotifyPropertyChanged` with no `class` and then subscribes to an event through it, which looked like the shape that made IL2CPP refuse `MultiColumnLayoutTracker.Attach` (#253). Settled by evidence rather than by reading: the IL2CPP build that proves the #253 fix converts `Ruitk.Shared.dll`, and the generated C++ contains `PropsHelper_Bind_m..._gp_0_0_0_0` / `_gp_1_0_0_0` for every overload - both generic parameters emitted, no error. An event subscription through a constrained type parameter is not the same emit as a property assignment through one. The public constraint stays as it is, which is the right outcome: narrowing it on suspicion would have been a breaking change for nothing. | `Shared/Props/PropsHelper.cs:138`, `:171`; `scripts/unity-il2cpp-check.mjs` | #253 audit 2026-09-20, closed 2026-09-21 |
| VNODE-POOL | The VirtualNode pool is DORMANT and ambiguous: `__Rent` allocates every time, nothing calls `__ScheduleReturn`, so `s_pool` is always empty and `__Reset` never runs. That is load-bearing now. `__Reset` sets `_children` to the shared empty list, and the #252 fix makes `FiberFragment.UpdateFragment` / `FiberReconciler.UpdatePortal` read an empty children list as "delete my children" - correct while a fiber can only hold a live node, wrong the moment a recycled one reaches it. Decide it deliberately: wire the returns up and revisit both guards together, or delete `__ScheduleReturn`/`__FlushReturns` and the pool with them. Leaving it dormant means the fix carries a standing assumption. Reciprocal comments are in place at all three sites. | `Shared/Core/VNode.cs` `__ScheduleReturn`; `Shared/Core/Fiber/FiberFragment.cs`; history: `def3c6e7` removed the returns and widened the bailout in one diff | #252, 2026-09-20 |
| SG-HMR-LOOP | `@foreach`/`@for`/`@while` are emitted with DIFFERENT tree shapes by the two emitters: the source generator returns `Func<VirtualNode[]>` and `__C` splices the items straight into the parent's child list, while the HMR emitter returns `Func<VirtualNode>` wrapping them in `V.Fragment(key: null, ...)`. Rendered output matches (fragments are host-less), but sibling POSITIONAL IDENTITY does not: `[a, i0, i1, b]` versus `[a, Fragment[i0,i1], b]`, so a keyless loop beside keyless siblings reconciles on different indices in the Editor and in a build. `HmrEmitterParityContractTests` does not cover loop emission at all - grep it for `foreach` or `Fragment` and nothing comes back. Pick one shape, make both emitters use it, and extend the parity contract to loops. | `SourceGenerator~/Emitter/CSharpEmitter.cs:2095,2118,2141` vs `Editor/HMR/HmrCSharpEmitter.cs:2058,2080` | #252 layer sweep, 2026-09-20 |
| DIST-TILDE | Most `~` folders still SHIP to UPM consumers. `publish.yml` rsyncs the whole tree and then deletes only what the two omit lists name, and neither names `SourceGenerator~/`, `ide-extensions~/`, `Plans~/`, `Samples~/` or `SharedTests~/` - so a git-URL install downloads the source generator sources, the LSP server, three IDE extensions and the whole plans tree. `~` hides a folder from Unity, not from the packer. Not changed during the #252-#259 wave: it alters what ships, which is the owner's call, not a bug fix. | `config.json` `pathsToOmitFromDist`; `.github/workflows/publish.yml` "Build dist" | #256 audit, 2026-09-20 |
| PKG-DEPS | `package.json` under-declares its dependencies. `Ruitk.Ugui.asmdef` references `UnityEngine.UI` and `Unity.TextMeshPro` (`com.unity.ugui`), and `Ruitk.Samples.asmdef` references those plus `Unity.InputSystem` (`com.unity.inputsystem`), while `dependencies` names only `com.unity.nuget.newtonsoft-json`. Install the package into a project without them and Unity drops the missing asmdef references with a warning, then those assemblies fail to compile - which puts the whole editor in "Scripts have compiler errors" and blocks play mode. Both packages are in every Unity 6 template, which is why nobody has hit it. Two honest fixes, and the choice is the owner's: declare them (forces two installs on every consumer, for one sample) or split the InputSystem-dependent Doom sample into its own asmdef behind a `versionDefines`/`defineConstraints` pair, which is the standard Unity idiom and costs consumers nothing. Found by `scripts/unity-editor-compile.mjs` on its first run; that script's shell project adds both by hand and says so. | `package.json` `dependencies`; `Ugui/Ruitk.Ugui.asmdef`; `Samples/Ruitk.Samples.asmdef`; `Samples/Components/DoomGame/DoomGameScreen.hooks.uitkx:2` | #255 CI work, 2026-09-20 |
| SAMPLES-PLAYER | **FIXED in 0.20.0 by deletion.** A player build of any consumer project failed to compile: `Samples/Components/EditorControlsDemoFunc/EditorControlsDemoFunc.uitkx` used `<TwoPaneSplitView>`, whose `V.TwoPaneSplitView` factory is `#if UNITY_EDITOR` (`Shared/Core/V.cs:90`) because the control is `UnityEditor.UIElements.TwoPaneSplitView`. The file sat in `Ruitk.Samples`, which is all-platforms, and was not under a folder named `Editor`, so Unity compiled it into the player and the generator - unable to see the editor-only factory - reported `UITKX0109` and `CS0103`. `Samples/` is a normal package folder (only `Samples~/` is hidden) and neither omit list excluded it, so it shipped on both channels. The owner chose deletion over an editor-only home; the sample and its only importer, `Samples/Showcase/Editor/EditorUitkxEditorControlsDemoWindow.cs`, are both gone. Verified by a full Windows IL2CPP player build of the unmodified repository: RESULT=SUCCEEDED. **Standing hazard:** nothing stops the next editor-only tag from landing in an all-platforms sample; the player build is now scriptable (`scripts/unity-il2cpp-check.mjs`) but is not yet wired into any workflow. | `Shared/Core/V.cs:90`; `Samples/Ruitk.Samples.asmdef` `includePlatforms: []` | found + fixed 2026-09-21 |
| SG-HMR-USESIGNAL | The lowercase `useSignal` wrappers are emitted by the SOURCE GENERATOR only. `HmrCSharpEmitter` and the IDE virtual document emit no `useSignal` alias at all - grep either for it and nothing comes back - so a `.uitkx` that calls `useSignal` compiles under the generator and, on the face of it, cannot compile under HMR. Found while scoping the 0.21.0 conditional-emit change (which touches the one emitter that has it, so it does not widen the gap). Either the HMR path resolves the call some other way, or HMR has never been exercised on a file using signals; nobody has reported it either way. Settle it with a .uitkx that calls useSignal under a live HMR session before trusting either answer. | `SourceGenerator~/Emitter/CSharpEmitter.cs` EmitHookAliasWrappers vs `Editor/HMR/HmrCSharpEmitter.cs` | 0.21.0 signals split, 2026-09-21 |
...
| C-02 | Fresh mounts flag `Placement` on every created fiber, so each host is attached by its own effect AND could be re-attached by every host-less ancestor's placement walk. 0.21.1 neutralised the cost with a commit-phase guard (`fiber.Alternate == null` skips the walk — mount stays at 1 attach per host, measured 51/51 over 50 component-wrapped hosts). React removes the cause instead, in the render phase: `shouldTrackSideEffects` means children are never flagged on a fresh mount. Adopting that would delete the guard and shrink the mount effect list. Deliberately NOT folded into the 0.21.1 bug fix — it touches `FiberChildReconciliation.PlaceChild` and `CompleteWork`, a much wider blast radius than the defect warranted | `Shared/Core/Fiber/FiberReconciler.cs` `CommitPlacement` fresh-mount guard; `FiberChildReconciliation.PlaceChild`; `Plans~/GITHUB_ISSUES_266_267_PLAN.md` | ISSUE-266 |
...
| RT-BLD | `scripts/unity-compile-check.mjs` compiles only `Shared/` + `Runtime/`, so an `Editor/`- or `Builder/`-assembly compile error is invisible to CI. The builder is now shipped code and its compile errors have escaped twice; the manual recipe in the embedded-clone notes is the only cover, and it has to be rebuilt by hand each time. Trigger to revisit: the next `Builder/` or `Editor/` change that is not hand-compiled before deploy. Needs the extra refs `Ruitk.Language.Editor.dll`, `System.Collections.Immutable.dll`, `Newtonsoft.Json.dll`, and must tolerate the generated `Ruitk.Uitkx.*` namespaces those assemblies `using` (they exist only after the generator runs). **Harder blocker found 2026-09-20:** `Ruitk.Editor` references `Ruitk.Ugui` and HMR dereferences it (`HmrCSharpEmitter.cs:3726`, `typeof(global::Ruitk.Ugui.U)`), so compiling `Editor/` pulls in `Ruitk.Ugui`, which needs `UnityEngine.UI` and `Unity.TextMeshPro` - UPM packages that live in a PROJECT package cache, not in the editor install `unity-compile-check.mjs` discovers. The cheap standalone-csproj route therefore cannot reach `Editor/` at all; this needs Unity in batch mode against a real project, which is the same vehicle as the macOS CI job (see `Plans~/GITHUB_ISSUES_252_259_PLAN.md` 10). **PARTLY CLOSED 2026-09-20:** `scripts/unity-editor-compile.mjs` imports the repo as a local package into a throwaway project and compiles every asmdef in batch mode, Editor/ and Builder/ included, and asserts each asmdef produced an assembly (a missing .meta shows up here as a missing DLL). Wired into `.github/workflows/unity-editor.yml`, which is INERT until UNITY_EMAIL/UNITY_PASSWORD are set and is not one of the four required contexts. What remains: make it required once it has run green a few times. | `grep "return ['Shared', 'Runtime']" scripts/unity-compile-check.mjs` | UITKX2114 wave, 2026-09-04 |
