...
| UB-05a | Builder inline canvas editors (attr value, directive header, hook chip, style entry, islands) have no completion/colouring — the source pane got the full treatment (UB-05/25) but the six inline TextFields are plain. The single-line four have exact `(file, line, col)` derivations for LSP completion (the commit handlers already compute every splice input); the two multiline island editors have no clean column map mid-edit. Needs a live-request path that synthesizes the edited line into the didChange payload | `Builder/Editor/Canvas/CanvasView.uitkx` inline editors; position map in the UB-05 dossier (campaign transcript) | UI Builder campaign 2026-08-16, UB-05 partial |
