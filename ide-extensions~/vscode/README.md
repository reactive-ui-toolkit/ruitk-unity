...
### [1.12.0] - 2026-08-29
- Breaking: a component parameter written WITHOUT a default value is now required, and a call site that omits it is an error — `UITKX0115`. `int x` and `int x = 0` were indistinguishable to the generator (both emit `public int X { get; set; } = 0;`), so a caller that forgot a prop silently rendered `default(T)` with no diagnostic anywhere. The required set is read from the declared parameters rather than from the generated props class, which cannot answer the question, and the server publishes it live beside `UITKX0109`. Exempt: `MutableRef<T>` parameters (out-channels filled by `ref={x}`), hand-written C# props classes (no notion of "no default written"), and any tag whose declarant the file cannot resolve unambiguously — attribute validation falls open to the UNION of candidates, which is right for "is this attribute known" and exactly wrong for "is this attribute required". Measured over the package's bundled samples before the severity was accepted: two props on one call site, both genuine defects. SG suite 1890/1890, LSP suite 185/185.
- Feature: the 16 router hooks (`RouterHooks.UseNavigate`, `UseParams`, `UseBlocker`, `UseSearchParams`, …) joined the shared `HookRegistry`, so they now get hover documentation and completion — and, more importantly, the rules of hooks. `UseBlocker` composes `Hooks.UseEffect`, so calling a router hook inside an `@if` or a loop shifts effect ordering exactly as a conditional `useEffect` would; `UITKX0013`/`UITKX0014` report it instead of nothing. Their fiber-slot arity is zero, verified against every hook in the source rather than assumed. No new syntax: they are still written `RouterHooks.UseNavigate()` with no camelCase shorthand, and they are deliberately kept out of the bare-call exemption set, so a bare `UseNavigate()` still reports as an unresolved reference rather than degrading into a puzzling `CS0103`. SG suite 1892/1892, LSP suite 185/185.
- Fix: an installed source overlay is now authoritative for import resolution. `ImportScopeFacts` fell back to `File.Exists` / `File.ReadAllText` behind the overlay, so a host driving the language library from in-memory buffers — the RUITK Builder does exactly this — could still have a module read from the stale file on disk after it had been deleted, moved or renamed in the session. Import targets, companion discovery and existence checks all now go through the overlay when one is installed, and fall through to disk only for paths the overlay cannot answer. Affects any embedder supplying buffers rather than files; unchanged for ordinary file-backed use. SG suite 1892/1892, LSP suite 185/185.

### [1.11.0] - 2026-08-15
- Feature: language server gains the builder request surface shipping with Unity package 0.17.0 (RUITK UI Builder): four custom requests (`ruitk/schema`, `ruitk/hooks`, `ruitk/componentProps`, `ruitk/workspaceGraph` with import-specifier resolution), an unsaved-buffer overlay in the workspace index so open-editor content is visible to element/props/graph intelligence, and the `enableRichText` attribute on `Label` in the embedded schema (completion + hover + unknown-attribute checks). One-way UXML-to-UITKX conversion joins the shared language library. SG suite 1878/1878, LSP suite 180/180.

### [1.10.0] - 2026-08-10
- Breaking: the legacy `component`/`hook`/`module` wrapper keywords and the `@component`/`@props`/`@key`/`@inject` directives were removed (package 0.16.0) — `UITKX2320` is now an error naming the `UitkxMigrateImports --es-modules` codemod. Completions, hover, references, rename, semantic tokens, the TextMate grammar, and the embedded schema now speak only the plain-declaration dialect (`@namespace`/`@using` still parse but left completions); references/rename regexes learned the modern heads including generic hooks (`useSel<T>(...)`), and retired diagnostics `UITKX2107`/`2108`/`2109`/`0211` no longer fire. Feature: generic declaration heads are first-class across the LSP (parse, virtual docs, bridges, formatter, highlighting). Fix: the formatter wraps over-width plain declaration heads again (parameters one per line, tuple returns ahead of the name).
- Fix: three navigation dead-ends. Ctrl+click did nothing on a hook whose over-width declaration head the formatter wrapped — the name lives on the `)` continuation line, which go-to-definition, references, rename, and the workspace index now all recognize; go-to-definition on an imported member call site resolves through the import alias to the declaring file; and find-usages on the FIRST export of a member-only file (every `.style.uitkx`) works — a member-only file has no setup block, so the setup-variable scan no longer hijacks its collapsed one-line range with a same-line jump. Two deeper root causes fixed in the same area: the virtual document now source-maps every member declaration's NAME (it is user-authored source, not scaffold), so cursor-on-name resolves through Roslyn for definition, references, rename, hover, and semantic tokens; and the references/rename cross-file text supplements are no longer gated on peer virtual documents existing — cross-file results used to depend on which tabs happened to be open. SG suite 1844/1844, LSP suite 166/166.
- Fix: a declared value name was colored by the PascalCase-vs-camelCase heuristic (`export Style Container` vs `export Style container` read as different token kinds). Declaration heads now carry deterministic grammar scopes (value, util, and wrapped-continuation heads), and the now-mapped declaration name additionally gets true semantic coloring from Roslyn.
- Fix: four false diagnostics from the samples field pass. TabView's `selectedIndexChanged`/`activeTabChanged` and EnumField's `enumType` schema types now match the runtime surface (`TabIndexEventHandler`/`TabChangedEventHandler`, and the assembly-qualified-name STRING the adapter actually consumes) — each produced a false CS0029 on code the Unity build accepts. `UITKX0111` (unused parameter) honors the C# discard convention — an underscore-prefixed parameter (`_onCountChanged`) is deliberately unused. And the strict-import detector's dotted-access hint no longer suggests importing a peer UTIL: `Tick.CreateInitialState()` used to suggest the unrelated util `Tick` from another game's GameLogic, but a util binds a function, so `import { Tick }` can never satisfy `Tick.member`; the hint remains for importable values. SG suite 1847/1847, LSP suite 171/171.
- Feature: a leading underscore on a component parameter marks it deliberately unused (silencing `UITKX0111`) WITHOUT renaming the public prop — `Action<int>? _onChanged = null` still exposes the `onChanged` attribute, so marking a parameter unused can never break consumers (field find: the rename-to-`_` suppression turned every consumer attribute into `UITKX0109`). One shared definition (`FunctionParam.PropSourceName`) drives the generated props class, the SG props binding, HMR's reflective prop lookup, and the editor's attribute checks and completions. Declaring both `_x` and `x` is ambiguous — new diagnostic `UITKX0114`. SG suite 1851/1851, LSP suite 172/172.
- Feature: import specifiers accept single quotes — `import {container} from './Foo.style';` now parses identically to the double-quoted form across every import shape (named, star, default, combined, and `import '@Ns'`), with the closing quote required to match the opener. The formatter canonicalizes to double quotes on save. Grammar highlighting, specifier-path completion, and HMR's import fan-out all recognize both styles. SG suite 1855/1855, LSP suite 172/172.
- Feature: `.uitkx` files show the Reactive UI Toolkit icon in the Explorer, editor tabs, and Open Editors (language icon contribution — parity with the Unreal leg's `.uetkx`).
